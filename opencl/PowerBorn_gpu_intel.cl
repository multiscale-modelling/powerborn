...
)
{
    ...
    for(int atomLoopId =integrationBlockId*GPU_BLOCK_SIZE/WARP_SIZE; atomLoopId <localConstants.atomCnt; atomLoopId += integrationBlockCount*GPU_BLOCK_SIZE/WARP_SIZE) //switch to atomic queue - save some mus.
    {
		int atomId = atomLoopId + (threadIdx / WARP_SIZE);
		...
		if (atomId < localConstants.atomCnt)
		{
		...
		if (atomId < localConstants.atomCnt)
		{
			...
			a = atoms[atomId + localConstants.atomsOffset];
        ...
        barrier(CLK_LOCAL_MEM_FENCE);
		if (threadIdx < GPU_BLOCK_SIZE / 8)
        {
			scratch[threadIdx * 2] += scratch[threadIdx*2 + 1];
        }
        barrier(CLK_LOCAL_MEM_FENCE);
		if (threadIdx < GPU_BLOCK_SIZE / 16)
		{
			scratch[threadIdx * 4] += scratch[threadIdx * 4 + 2];
		}
		barrier(CLK_LOCAL_MEM_FENCE);
		if (threadIdx < GPU_BLOCK_SIZE / 32 && atomId + threadIdx < localConstants.atomCnt)
        {
			tempIntegrals[atomId + threadIdx + localConstants.atomsOffset*integrationBlockCount + integrationBlockId*localConstants.atomPitch] += scratch[threadIdx * 8] + scratch[threadIdx*8 + 4];
        }
...
)
{
    ...
    for(int wPosId = (*level1AtomIdId)+ threadIdx / WARP_SIZE; wPosId < level1AtomIdCnt; wPosId += GPU_BLOCK_SIZE_KERNEL2/WARP_SIZE)
    {
        int aId = stack[wPosId  + localConstants.stackOffset];
        float4 a = atoms[aId + localConstants.atomsOffset];
        float dx = px - a.x;
        dx=a.w*a.w - dx*dx;
        if(dx > 0)
        {
...
)
{
    ...
    // initial population of coarse grid
    {
	    ...
	    int atomstart;
	    int waterstart;
            if(render[id] > 0 && id < localConstants.maincellcount)
            {
                //compute stack alloc 
                if(id & 0x1)
                {
                ...
                atomstart = maincellPos + 4;
                waterstart = atomstart + render[id];
    ...
    if(localConstants.maxLevel > 8) //large grid - sort atoms on lower levels
    {
        ...
        while (level1CellDataOffset < level1CellDataTotal)
        {
	        ...
	        int atomstart;
	        int waterstart;
                if(render[id] > 0 && id < 64*level1IntegIdCnt)
	        {
                    //compute stack alloc 
                    if(id & 0x1)
                    {
...
// this needs to be fixed to match code below
...

            {
                ...
                while( atomIdId <level1IntegIdCnt)
                {
                    ...
                    if (threadIdx == 0) result[atomId + integrationBlockOffset] = res + (level3[0].w + level3[1].w);
                    barrier(CLK_LOCAL_MEM_FENCE);
                    atomIdId++;
                }
                barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);
